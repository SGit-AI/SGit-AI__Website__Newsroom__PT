#!/usr/bin/env python3
"""pt.newsroom.sgit.ai — the design gates (27-33, 39). What the review measured, now asserted.
...
# --- 39. the legacy layer is a ratchet, not a home ------------------------------
# console.css carries a block that maps the PAPER's class names onto the console's — .chip onto
# the four-rank status scale, .cartao onto a card, .sect onto a field label. It exists because the
# back office is generated by modules that emit those names, and rewriting all of them in the
# release that adopted the console would have been a far larger diff than the review itself. The
# block's own comment says its end state is being deleted.
#
# WHAT IS COUNTED, AND WHY IT IS NOT THE USAGES. The first version of this gate counted every
# occurrence of a legacy class in the built HTML and held it under a ceiling. It went red on the
# next commit — because that commit added twelve items to the design-review page, and each item is
# a card with two paragraphs. The number had gone up for the right reason and the gate could not
# tell. A metric that rises when the newsroom publishes more is not measuring the bridge.
#
# So what is counted is the GENERATORS: how many modules still emit the paper's class names into a
# back-office page. That number is independent of how much those modules print, it falls only when
# one is actually converted, and its end state is zero — at which point the legacy block in
# console.css is deleted and this gate goes with it.
#
# Lower this number when you convert a generator. Never raise it.
TETO_DE_GERADORES_LEGADOS = 4
...
if len(legados) > TETO_DE_GERADORES_LEGADOS:
    erros.append(
        ...
        f"contra um teto de {TETO_DE_GERADORES_LEGADOS} \u2014 {', '.join(legados)}. A camada "
        f"legada de console.css \u00e9 uma ponte com fim declarado: um gerador novo emite .st / "
        f".card / .bo. Se converteu um, baixe o teto no mesmo commit")
...
print("portões do desenho: OK — " + ", ".join(notas[:6]) +
      ...
      f"camada legada em {len(legados)} de {len(GERADORES_DOS_BASTIDORES)} geradores dos "
      f"bastidores, sob o teto de {TETO_DE_GERADORES_LEGADOS}")
