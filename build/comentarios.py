...
# Os agentes desta redação vêm de `dados/agentes.json`, que é o registo, e não de uma lista aqui.
# Havia duas listas — esta e o registo — e duas listas de agentes acabariam por discordar no dia em
# que alguém acrescentasse um papel a uma e não à outra. A chave continua a ser o `id_curto`,
# porque é nele que estão as entradas de comentário de cada artigo, e essas são prova.
def _agentes_do_registo():
    ...
    return {
        a["id_curto"]: {
            ...
            # O endereço de correio do mesmo agente, para que quem lê o mapa de comentários possa
            # ir do que ele fez para o que ele tem à frente.
